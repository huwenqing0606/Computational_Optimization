...
import numpy as np
from src.activations.activations import Sigmoid, ReLU
import os
...
class fullnetwork(object):
    ...
    def setparameter(self):
        ...
        #   according to the LeCun initialization
        weight = []
        bias = []
        # the initial layer
        weight.append(
            np.array(
                [
                    [np.random.normal(loc=0.0, scale=1.0) for i in range(1)]
            ...
            if l == self.L - 1:
                ...
                weight.append(
                    np.array(
                        [
                            [
                                np.random.normal(loc=0.0, scale=1 / np.sqrt(self.n[l]))
