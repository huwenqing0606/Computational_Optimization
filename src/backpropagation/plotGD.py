...
# the two weights taken from randomly sample
#   two neurons from each of the above layers,
#   from [1, width of that layer]
weightindex_neuron_startlayer = np.random.randint(
    1, n[weightindex_startlayer - 1] + 1, size=2
)
weightindex_neuron_nextlayer = np.random.randint(
    1, n[weightindex_nextlayer - 1] + 1, size=2
)
...
def plot_gd_trajectory(w1_init, w2_init, learningrate):
    ...
    weight[weightindex_startlayer][weightindex_neuron_nextlayer[0] - 1][
        weightindex_neuron_startlayer[0] - 1
    ] = w1_init
    weight[weightindex_startlayer][weightindex_neuron_nextlayer[1] - 1][
        weightindex_neuron_startlayer[1] - 1
    ] = w2_init
    ...
    for i in range(N):
        ...
        gradweight, _gradbias = backprop.grad(x, delta)
        ...
        weight[weightindex_startlayer][weightindex_neuron_nextlayer[0] - 1][
            ...
            w_1[i]
            - learningrate
            * gradweight[weightindex_startlayer][weightindex_neuron_nextlayer[0] - 1][
                weightindex_neuron_startlayer[0] - 1
            ]
        )
        weight[weightindex_startlayer][weightindex_neuron_nextlayer[1] - 1][
            ...
            w_2[i]
            - learningrate
            * gradweight[weightindex_startlayer][weightindex_neuron_nextlayer[1] - 1][
                weightindex_neuron_startlayer[1] - 1
            ]
        )
        networkoutput, outputsequence, preoutputsequence = network.output(
...
if __name__ == "__main__":
    ...
    def anmi(i):
        ...
        point = ax.plot(
            w_1[i - 1 : i], w_2[i - 1 : i], Loss[i - 1 : i], "bo", markersize=10
        )
        return line, point
    ...
    animfilename = (
        ...
        + "_neuron"
        + str(weightindex_neuron_startlayer[0])
        + str(weightindex_neuron_nextlayer[0])
        + "_neuron"
