...
class backpropagation(object):
    ...
    def error(self, y):
        ...
        delta[self.L + 1 - 1] = np.dot(
            self.activation.grad(self.preoutputsequence[self.L + 1 - 1]),
            self.outputsequence[self.L + 1 - 1] - np.array(y),
        )
        # backpropagation: from the last layer to the first hidden
        #   layer calculate all the error functions
        for i in reversed(range(self.L)):
            vector1 = np.dot(self.weight[i + 1].T, delta[i + 1])
            vector2 = []
            for index in range(self.n[i]):
                vector2.append(
                    self.activation.grad(self.preoutputsequence[i][index][0])
                )
    ...
    def grad(self, x, delta):
        # calculation of the gradient of the quadratic
        #   loss with respect to the weight and bias parameters
        ...
        #   this will be an array of the same size as the weights
        gradweight = []
        # the initial layer
        gradweight.append(
            np.array(
                [
                    [float(delta[0][j]) * float(x) for k in range(1)]
...
if __name__ == "__main__":
    ...
    # calculate the network output and the layer-by-layer output
    #   sequence and preoutput sequence
    x = 1
    y = 1
