...
# the two weights taken from randomly sample two
#   neurons from each of the above layers,
...
def plot_network_loss():
    ...
    for i in range(N):
        for j in range(N):
            ...
            # calculate the mean square error produced by
            #   the weight parameters at (a_1[i], a_2[j])
            Z = []
            for m in range(training_size):
                networkoutput, outputsequence, preoutputsequence = network.output(
                    float(X[m]), weight, bias
                )
