...
\E\|x^k-x^*\|^2\leq \left(1-2m\al+\al^2L_g^2\right)^k\|x^0-x^*\|^2+\dfrac{\al B^2}{2m-\al L_g^2} \ .
\end{equation}
\end{theorem}
...
We see from (\ref{Thm:ConvergenceSGD:Eq:ConvergenceStronglyConvex}) that there is a ``threshold value" $\dfrac{\al B^2}{2m-\al L_g^2}$ which prevents SGD to converge (i.e. there is always a fluctuation). To over comes this, ``epoch doubling" and ``hyper-parameter tuning" have been used. 
...
As we have seen in Theorem \ref{Thm:ConvergenceSGD}, the ``threshold value" $\dfrac{\al B^2}{2m - \al L_g^2}$ prevents SGD to converge. This is due to the ``variance term" $B>0$. This inspires Variance Reduction, whose goal is to reduce the variance produced by the stochastic terms in the iteration scheme.
...
Each row corresponds to one epoch of length $m$, with $\widetilde{w}_s = w^{((s-1)m)}$
...
Consider the SVRG \emph{Algorithm \ref{Alg:SVRG}} and assume that each $\grad \psi_i$ is $L$--Lipschitz and convex, and the function
...
\subsection{SAG, SAGA, SARAH, SPIDER, STORM}
