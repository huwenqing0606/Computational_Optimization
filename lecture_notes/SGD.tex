...
Some other cases can also be discussed similarly:
\begin{itemize}
\item $L_g=0$. Let the average output $\bar{x}^K=\dfrac{\sum\li_{j=0}^K \al_jx^j}{\sum\li_{j=0}^K \al_j}$. Set $D_0=\|x^0-x^*\|^2$,  $\al_k=\al>0$ and $\al_{\text{opt}}=\dfrac{D_0}{B\sqrt{T+1}}$, $\tht=\dfrac{\al}{\al_{\text{opt}}}$. If $L_g=0$, then
