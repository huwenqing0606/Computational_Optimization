...
\begin{proof}
Use fixed point iteration. Let $\Phi(x)=x-\al \grad f(x)$, $\al>0$. Then $x^{k+1}=\Phi(x^k)$.
Calculate $\|\Phi(x)-\Phi(z)\|\leq \bt\|x-z\|$.

In order $\|x^T-x^*\|\leq \ve$ we want $T\geq \dfrac{\ln\left(\dfrac{\|x^0-x^*\|}{\ve}\right)}{|\ln \bt|}$.

Want $-\bt\leq 1-\al L\leq 1-\al m\leq \bt$, and when $1-\al L =1-\al m$, we have $\bt=\dfrac{L-m}{L+m}$ and
$\al=\dfrac{2}{L+m}$.
\end{proof}
