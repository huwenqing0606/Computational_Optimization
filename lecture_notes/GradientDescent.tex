...
Classical algorithm of training multi--layer neural networks: Back--propagation. This algorithm was proposed in \cite{HintonEtAlBackPropagation}. For a modern approach with the flavor of applier mathematics, we refer to \cite{HighamDeepLearningAppliedMath}. 
...
\begin{tikzpicture}[
  ...
  conn/.style={-Latex, line width=0.45pt},
  font=\small
]
...
% Input node
\node[dot] (a1) at (\xin,0) {};
\node[below=2pt] at (a1) {$a^{[1]}$};

% Layer 2
\node[dot] (L2a) at (\xone,\yA) {};
\node[dot] (L2b) at (\xone,\yB) {};
\node[dot] (L2c) at (\xone,\yC) {};
\node[dot] (L2d) at (\xone,\yD) {};
\node[below=6pt] at (\xone,-1.55) {$a^{[2]}$};

% Layer 3
\node[dot] (L3a) at (\xtwo,\yA) {};
\node[dot] (L3b) at (\xtwo,\yB) {};
\node[dot] (L3c) at (\xtwo,\yC) {};
\node[dot] (L3d) at (\xtwo,\yD) {};
\node[below=6pt] at (\xtwo,-1.55) {$a^{[3]}$};

% Middle dots
\node at (\xmid,0) {$\cdots$};

% Layer L-1
\node[dot] (LLa) at (\xlast,\yA) {};
\node[dot] (LLb) at (\xlast,\yB) {};
\node[dot] (LLc) at (\xlast,\yC) {};
\node[dot] (LLd) at (\xlast,\yD) {};
\node[below=6pt] at (\xlast,-1.55) {$a^{[L-1]}$};

% Output node
\node[dot] (aL) at (\xout,0) {};
\node[below=2pt] at (aL) {$a^{[L]}$};

% Fully-connected edges: a1 -> Layer2
\foreach \t in {L2a,L2b,L2c,L2d} {
  \draw[conn] (a1) -- (\t);
}

% Fully-connected edges: Layer2 -> Layer3
\foreach \s in {L2a,L2b,L2c,L2d} {
  \foreach \t in {L3a,L3b,L3c,L3d} {
    \draw[conn] (\s) -- (\t);
  }
}

% Fully-connected edges: Layer(L-1) -> aL
\foreach \s in {LLa,LLb,LLc,LLd} {
  \draw[conn] (\s) -- (aL);
}

% Labels above CONNECTIONS (midpoints)
\node at ({(\xin+\xone)/2}, -1.5) {$W^{[2]},\, b^{[2]}$};
\node at ({(\xone+\xtwo)/2}, -1.5) {$W^{[3]},\, b^{[3]}$};
\node at ({(\xlast+\xout)/2}, -1.5) {$W^{[L]},\, b^{[L]}$};

\end{tikzpicture}
\caption{Feedforward fully-connected neural network (intermediate layers omitted).}
\end{figure}
...
See \cite{Zhang2018ReLU, Du2018OverparamGD, AllenZhu2018Convergence, Du2018GlobalMin}.

Computational Graph and General Backpropagation for taking derivatives: basic idea in tensorflow (see \cite{TensorflowAutodiff}). 

\noindent{\textbf{Example.}} Consider
\[
 f(x)=\ln\bigl(\sin^2(x^2)\bigr),
\]
defined for $\sin(x^2)\neq 0$. Introduce intermediate variables
\begin{equation}
 u=x^2,\qquad v=\sin(u),\qquad w=v^2,\qquad f=\ln(w).
 \label{eq:forward-defs}
\end{equation}
...
\node (x) {$x$};
\node (u) [right=of x] {$u=x^2$};
\node (v) [right=of u] {$v=\sin(u)$};
\node (w) [right=of v] {$w=v^2$};
\node (f) [right=of w] {$f=\ln(w)$};
...
% local derivatives
\node (du) [below=8mm of u, draw=none] {$\dfrac{\partial u}{\partial x}=2x$};
\node (dv) [below=8mm of v, draw=none] {$\dfrac{\partial v}{\partial u}=\cos(u)$};
\node (dw) [below=8mm of w, draw=none] {$\dfrac{\partial w}{\partial v}=2v$};
\node (df) [below=8mm of f, draw=none] {$\dfrac{\partial f}{\partial w}=\dfrac{1}{w}$};
...
Define adjoints $\bar{u}=\partial f/\partial u$, $\bar{v}=\partial f/\partial v$,
$\bar{w}=\partial f/\partial w$, and $\bar{x}=\partial f/\partial x$.
Start from the output:
\[
\bar{f}=1,\qquad \bar{w}=\bar{f}\,\frac{\partial f}{\partial w}=\frac{1}{w}.
\]
Propagate backward along the edges using the chain rule:
\begin{align*}
\bar{v} &= \bar{w}\,\frac{\partial w}{\partial v}=\frac{1}{w}(2v)=\frac{2}{v},\\
\bar{u} &= \bar{v}\,\frac{\partial v}{\partial u}=\frac{2}{v}\cos(u)
        =2\frac{\cos(u)}{\sin(u)}=2\cot(u),\\
\bar{x} &= \bar{u}\,\frac{\partial u}{\partial x}=\bigl(2\cot(u)\bigr)(2x)=4x\cot(u).
\end{align*}
Substituting $u=x^2$ gives
\begin{equation}
 f'(x)=\frac{\mathrm d f}{\mathrm d x}=4x\cot(x^2), \qquad \sin(x^2)\neq 0.
\end{equation}

This illustrates the key idea of automatic differentiation: evaluate the forward pass
\eqref{eq:forward-defs} once, then compute derivatives by a single backward pass using
local Jacobians at each node.

In PyTorch, all computational flows are stored during calculation, so that any gradient can be calculated using back-propagation. This takes memory. So \verb#torch.no_grad()# (inference mode) can be used to accelerate the inference process if no gradient computation is needed.
...
We can design an online version of the PCA algorithm as in Algorithm \ref{Alg:OnlinePCA}, where we must do at every iteration a normalization step using the retraction operation 
$$\Pi \boldu=\dfrac{\boldu}{\|\boldu\|} \ .$$
...
The convergece of online PCA has been proved only recently in \cite{OnlinePCAMathProgramming}. The difficulty here is that this online PCA is non-convex optimization.

Oja's algorithm: Step 4 in Algorithm \ref{Alg:OnlinePCA} will be modified to
$$\boldu^{(n)}=\boldu^{(n-1)}+\bt\left( \boldX^{(n)}(\boldX^{(n)})^T\boldu^{(n-1)}-\left((\boldu^{(n-1)})^TX^{(n)}\right)^2\boldu^{(n-1)}\right) \ ,$$
which is expected to control the norm growth. See \cite{Oja1982}.
...
\\
&=f(x^k)+\al \grad f(x^k)^T d^k+\al \play{\int_0^1 [\grad f(x^k+\gm\al d^k)-\grad f(x^k)]^Td^kd\gm}
\\
&\leq f(x^k)-\al \left(\bar{\ve}-\al\dfrac{L}{2}\gm_2\right)\|\grad f(x^k)\|\cdot \|d^k\| \ .
\end{array}
\end{equation}

So if $\al\in \left(0, \dfrac{2\bar{\ve}}{L\gm_2}\right)$, then $f(x^{k+1})<f(x^k)$, leading to a descent in objective value.
...
\begin{itemize}
\item[(1)] $d^k=-S^k \grad f(x^k)$, $S^k$ symmetric positively definite, $\lb(S^k)\in [\gm_1, \gm_2]$;

In this case we have $-(d^k)^T \grad f(x_k)=(\grad f(x_k))^T S^k (\grad f(x_k))\geq \gm_2 \|\grad f(x^k)\|^2\geq \gm_2\gm_2^{-1}\|\grad f(x^k)\|\cdot \|d^k\|=\|\grad f(x^k)\|\cdot \|d^k\|$.

\item[(2)] Gauss-Southwell: $d^k=-[\grad f(x^k)]_{i_k}$ where $$i_k=\arg\max\li_{i=1,2,...,n}\left|[\grad f(x^k)]_i\right|$$ and $$\grad f(x) = ([\grad f(x)]_1, ..., [\grad f(x)]_n) \ ;$$

\item[(3)] Stochastic coordinate descent: $d^k=-[\grad f(x^k)]_{i_k}$ where $i_k$ is uniformly randomly drawn from $\{1,2,...,n\}$; 

\item[(4)] Stochastic Gradient Methods: $d^k$ is random, we will specify it later in Chapter 5; 

\item[(5)] Exact line search $\min\li_{\al>0}f(x^k+\al d^k)$; 

\item[(6)] Approximate Line Search: choose a certain $\al>0$, not necessarily the exact $\al$ in the above. This usually is accompanied with the ``weak Wolfe conditions", which consists of (a) Armijo (Sufficient Decrease) Condition: $f(x^k+\al p^k)\leq f(x^k)+c_1 \al_k (p^k)^T \grad f(x^k)$, where $c_1\in (0,1)$ and is often chosen to be of a small order of magnitude around $10^{-3}$. This condition ensures the computed step length can sufficiently decrease the objective function $f(x^k)$. Only using this condition, however, it cannot be guaranteed that $x^k$
 will converge in a reasonable number of iterations, since Armijo condition is always satisfied with step length being small enough. Therefore, the second condition below needs to be paired with the sufficient decrease condition to keep $\al_k$
 from being too short. (b) Curvature condition: $(\grad f(x^k+\al p^k))^T p^k \geq c_2 (\grad f(x^k))^T p^k$ where $c_2\in (c_1, 1)$ is much greater than $c_1$ and is typically on the order of $0.1$. The condition ensures a sufficient increase of the gradient; 

\item[(7)] Backtracking Line Search: Starting from some $\bar{\al}>0$ then look for $\al \in \left\{\bar{\al}, \bt \bar{\al}, \bt^2\bar{\al}, ... \right\}$ where $0<\bt<1$.
\end{itemize}
...
\section{Experiment: Learning a Neural Network via Gradient Descent and Backpropagation}

Consider an $L$-hidden-layer fully connected neural network with hidden layer sizes $n_1, ..., n_L$, where the input layer has only one neuron $x\in \R$
 and output layer has only one neuron $y\in \R$. Let the training data $(x_1, y_1), ..., (x_N, y_N)$ be an i.i.d sample of a bivariate unit normal distribution $(x,y)\in \cN(0, I_2)$. We use the backpropagation algorithm to calculate the gradient vector of the empirical loss function and the population loss function of the neural network for a quadratic loss.

We experiment 

\begin{itemize}
\item[(1)] Different hidden layer sizes: $(n_1, ..., n_L)$;

\item[(2)] Different number of hidden layers: $L$;

\item[(3)] Different sizes of the training data set: $n$;

\item[(4)] Different activation functions: Sigmoid, ReLU, Tanh.
\end{itemize}  
...
Code can be found at \url{https://github.com/huwenqing0606/Computational_Optimization/tree/main/src/backpropagation}
