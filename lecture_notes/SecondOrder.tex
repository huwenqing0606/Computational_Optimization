...
\emph{Theorem \ref{Thm:NecessaryConditionsSmoothUnconstrainedOptimization} (b)},
...
\begin{proof}
See the proof of \cite[Theorem 3.5]{NocedalWrightNumericalOptimization}. Since $x^*$ is second-order stationary, we have $\nabla^2 f(x^*) \succeq \mu I$ for some $\mu>0$, so the matrix $\nabla^2 f(x^*)$ is invertible.
By continuity of $\nabla^2 f$, if $x_k$ is sufficiently close to $x^*$, then $\nabla^2 f(x_k)$
...
We now estimate the Newton error. Since $\nabla f(x^*)=0$, we write
\begin{align*}
x_{k+1}-x^*
&=
x_k-x^*-[\nabla^2 f(x_k)]^{-1}\bigl(\nabla f(x_k)-\nabla f(x^*)\bigr) \\
...
=
\int_0^1
\nabla^2 f\bigl(x_k+\gamma(x^*-x_k)\bigr)(x_k-x^*)\,d\gamma.
\]
Hence
\begin{align*}
&\nabla^2 f(x_k)(x_k-x^*)
...
\nabla^2 f(x_k)
-
\nabla^2 f\bigl(x_k+\gamma(x^*-x_k)\bigr)
\Bigr](x_k-x^*)\,d\gamma.
\end{align*}
Therefore
\begin{align*}
...
\Bigr](x_k-x^*)\,d\gamma.
\end{align*}
Taking norms and using the Hessian Lipschitz condition,
\begin{align*}
\|x_{k+1}-x^*\|
&\le
\|[\nabla^2 f(x_k)]^{-1}\|
...
\frac12\|x_0-x^*\|,
\]
and inductively the iterates remain in the local neighborhood and converge to $x^*$.
Since the error satisfies a quadratic recursion, the convergence is quadratic. In particular,
\[
\frac{\|x_{k+1}-x^*\|}{\|x_k-x^*\|}
\le
\alpha \|x_k-x^*\| \to 0,
\]
so the convergence is also superlinear.
...
\begin{enumerate} 
\item[(1)] Inverse of Hessian $[\grad^2 f(x)]^{-1}$ hard to calculate for high--dimensional problems;

\item[(2)] No positive definite guarantee of Hessian $\grad^2 f(x)$.
\end{enumerate}

\section{Quasi--Newton: BFGS}

Reference: \cite{DennisMoreQuasiNewtonSIAMReview}.
...
It can be shown that when $f$ is strongly convex then \eqref{Eq:CurvatureCondition} is satisfied. 
...
Superlinear convergence of BFGS method (see \cite[Theorem 6.6]{NocedalWrightNumericalOptimization}).

\section{Stochastic Quasi--Newton}

Reference: \cite{ByrdHansenNocedalStochasticQuasiNewtonSIAMOptimization}.
